...
def count_message(from_username, to_username):
    query = """SELECT ID, FROM_USERNAME, TO_USERNAME, CREATED_AT, TEXT, MEDIA_URL
            ...
            """
    data = (from_username, to_username, to_username, from_username)
    array = db_manager.execute_query(query, data)
    length = len(array)
    return length
...
@pytest.mark.parametrize("test_username, expected_output, last_message_retrieved_id",
                         [
                             ('ahly', 2, None),
                             ('ahly', 1, get_id('ahly', 'zamalek')),
                         ])
def test_get_messages(test_username, expected_output, last_message_retrieved_id):
    """
        this function tests if list of messages is returned properly by checking size of the list
    """
    list_conversations = actions.get_conversations(test_username, last_message_retrieved_id)
    count = len(list_conversations)
    assert count == expected_output
...
def test_get_messages_value_error():
    ...
    exception_caught = False
    try:
        actions.get_messages('ahly', 'invalid_id')
    except ValueError:
        exception_caught = True
    assert exception_caught is True
...
def test_get_messages_empty():
    ...
    """
    list_messages = actions.get_messages('no_message', None)
...
def test_create_message_to_user_error():
    ...
    """
    try:
        actions.create_message('zamlek', 'qq', 'test message',None)
    except Exception as E:
