...
class Media(Resource):
    ...
    @kweeks_api.response(code=201, description='Image has been uploaded successfully',
                         model=create_model('Media ID',
                                            model={'media_id':
                                                   fields.String(description='The id of the uploaded image.')}))
    @kweeks_api.response(code=404, description='Media not found. Uploading failed.')
    @kweeks_api.response(code=401, description='Unauthorized access.')
    def post(self):
            """
